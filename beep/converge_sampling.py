...
def sampling(method, basis, program, opt_lot, kw_id, num_struct, rmsd_symm, wat_collection, wat_cluster, smol_name, database, sampling_shell, o_file, client):
...
try:
        ...
        print_out("Molecules to be sampled are not optimized at the requested level of theory, please optimize them first")
   ...
   #     database = database
    ...
    try:
        ...
        '''.format(spec['name'], smpl_database)
    except KeyError:
    ...
    while not converged:
        ...
        print_out('''Initiating round {}: Generated {} structures to be optimized:
    Molecule Hash:
    {}
    
        '''.format(c, num_struct, mol_l)
        )
        ...
        print_out(''' The procedure IDs of round {} molecules are:
    {}
    ...
    structure will be compared with all of the existing structures'''.format(database))
    ...
    ************************** Starting  RMSD Filtering procedure ************************** 
    ...
    '''.format(complete_opt_name))
        ...
        for n in complete_opt_name:
            ...
            if not ds_opt.data.records:
                print_out("Seeding the OptimizationDataset with the first structure {} \n".format(n))
    ...
    """.format(n, rmsd_val)
            ...
            for j in id_list:
                ...
                rj = client.query_results(molecule=j[1].initial_molecule, method = method)[0]
    ...
    """.format(n , j[0], rmsd_val_map, rmsd_val_mirror)
                out_string+=""" 
        
        Comparing molecule {}  with molecule {} the RMSD values is: {}
        
    """.format(n , j[0], rmsd_val_map)
    ...
    ****************************************End of round***************************************
    ...
    '''.format(len(complete_opt_name), len(ds_opt.df.index), c, new, tot_mol)
        print_out(out_string)
        c = c+1
        ...
        if (len(ds_opt.df.index) >= max_struct) or c >= 7 or (new <= 1):
            converged=True
            print_out("All or at least {} binding sites were found! Exiting...".format(max_struct))
            return converged
