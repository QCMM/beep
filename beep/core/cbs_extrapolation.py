...
def scf_xtpl_helgaker_2(functionname: str, zLO: int, valueLO, zHI, valueHI, verbose: int = 0, alpha = None):
    ...
    if isinstance(valueLO, float):
        ...
        raise ValueError(f"scf_xtpl_helgaker_2: datatype is not recognized '{type(valueLO)}'.")
...
def scf_xtpl_helgaker_3(functionname, zLO, valueLO, zMD, valueMD, zHI, valueHI, verbose=False, alpha=None):
    ...
    if (type(valueLO) != type(valueMD)) or (type(valueMD) != type(valueHI)):
        raise ValueError("scf_xtpl_helgaker_3: Inputs must be of the same datatype! (%s, %s, %s)" %
