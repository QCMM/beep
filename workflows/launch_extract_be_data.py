...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        default=["WB97X-V", "M06-HF", "WPBE-D3MBJ"],
        help="Binding energy methods",
    )
    parser.add_argument(
        "--server-address",
        type=str,
        default="https://152.74.10.245:7777",
        help="Server address",
    )
    parser.add_argument("--mol-coll-name", type=str, help="Molecule collection name")
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=[],
        help="List of clusters on which the Hessian was computed",
    )
    parser.add_argument(
        "--molecules", type=str, nargs="*", default=[], help="List of molecules"
    )
    parser.add_argument(
        "--be-range",
        type=float,
        nargs=2,
        default=[-0.1, -25.0],
        help="Binding energy range to filter structures (default: [-0.1, -25.0] kcal/mol)",
    )
    parser.add_argument(
        "--scale-factor",
        type=float,
        default=0.958,
        help="Scaling factor for ZPVE corrections (default: 0.958)",
    )
    parser.add_argument(
        "--basis",
        type=str,
        default="def2-tzvp",
        help="Basis set for BE computation (default: def2-tzvp)",
    )
    parser.add_argument(
        "--exclude-clusters",
        type=str,
...
def concatenate_frames(
    ...
    for w in ds_w.df.index:
        ...
        try:
            ...
            logger.info(
                f"ReactionDataset {name_be} exists but seems to be empty, please check."
            )
            continue
...
def zpve_correction(
    ...
    for name in name_be:
        ...
        logger.info(
            f"Extracting and saving binding energies from {name} for ZPVE correction"
        )
    ...
    df_zpve = pd.DataFrame.from_dict(
        zpve_corr_dict, orient="index", columns=["Delta_ZPVE"]
    )
    ...
    if len(df_be) < 5:
        raise ValueError(
            "Too few Hessians to construct a ZPVE linear model. Please compute more Hessians."
        )
    if 5 <= len(df_be) <= 9:
        logger.info(
            f"WARNING: Number of Hessians is low and may result in a poor linear model. Proceed with caution."
        )
    else:
    ...
    for bm in be_methods:
        ...
        logger.info(
            f"Linear model at the {bm} level of theory: BE+ ΔZPVE = {m} * BE + {b}"
        )
...
def apply_lin_models(
    df_be: pd.DataFrame,
    df_be_zpve: pd.DataFrame,
    meth_fit_dict: Dict[str, List[float]],
    be_methods: List[str],
    basis: str,
    mol: str,
    be_range: Tuple[float],
) -> pd.DataFrame:
    """
...
def main():
    ...
    for mol in mol_list:
        ...
        en_log_mol = write_energy_log(
            res_be_lin_zpve, mol, en_log_mol, "(Linear model ZPVE):"
        )
        logger.info(en_log_mol)

        final_result_nz = write_energy_log(
            res_be_no_zpve, mol, final_result_nz, "(NO ZPVE):"
        )
        final_result_dz = write_energy_log(
            res_be_zpve, mol, final_result_dz, "(Direct ZPVE):"
        )
        final_result_lz = write_energy_log(
            res_be_lin_zpve, mol, final_result_lz, "(Linear model ZPVE):"
        )
